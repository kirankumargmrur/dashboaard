...
import com.capitalone.dashboard.model.QCIData;
...
public class DashboardServiceImpl implements DashboardService {
    ...
    private final CIDataRepository ciDataRepository;
    ...
    public DashboardServiceImpl(DashboardRepository dashboardRepository,
        ...
        this.ciDataRepository = ciDataRepository;
    ...
    public DataResponse<Iterable<CIData>> retrieveCIData(ObjectId collectorItemId) {
    	LOGGER.info("retrieveCIData entry");
    	String jobName = "";
    	CollectorItem collectorItem = collectorItemRepository.findOne(collectorItemId);
    	
        if (collectorItem == null) {
            Iterable<CIData> results = new ArrayList<>();
            return new DataResponse<>(results, System.currentTimeMillis());
        }
        LOGGER.info("retrieveCIData: found matching collectorItemId !");
        
    	Map<String, Object> options = collectorItem.getOptions();
    	jobName = options.get("jobName").toString();
    	
    	LOGGER.info("retrieveCIData: looking in ci_build_data collector for jobName: "+jobName);
    	
    	PageRequest pageRequest = new PageRequest(0, 10, Sort.Direction.DESC, "buildId");
    	List<CIData> result = new ArrayList<>();;
		//= ciDataRepository.findCIDatas(jobName, pageRequest);
		
		BooleanBuilder builder = new BooleanBuilder();
		builder.and(QCIData.cIData.jobName.eq(jobName));
		result.addAll(Lists.newArrayList(ciDataRepository.findAll(builder.getValue(), pageRequest).getContent()));
		LOGGER.info("retrieveMatchingDocuments: " + result);
		return new DataResponse<>(result, System.currentTimeMillis());

    }
    
	public void retrieveMatchingDocuments(String jobName) {

    	/*BasicDBObject whereQuery = new BasicDBObject();
    	whereQuery.put("jobName", jobName);
    	
    	try {

    		  @SuppressWarnings("deprecation")
			  Mongo mongo = new Mongo("localhost", 27017);
    		  DB db = mongo.getDB("dashboardb");

    		  // get a single collection
    		  DBCollection collection = db.getCollection("ci_build_data");
    		  DBCursor cursor = collection.find(whereQuery);
    		  while (cursor.hasNext()) {
    			  DBObject result= cursor.next();
    			  JSONObject output = new JSONObject(new JSON().serialize(result)); 
    		  }
    	}
    	catch (Exception e) {
    		e.printStackTrace();

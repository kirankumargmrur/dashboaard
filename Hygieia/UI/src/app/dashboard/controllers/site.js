    ...
    function SiteController($scope, $q, $uibModal, dashboardData, $location, DashboardType, userService,
        ...
        (function() {
            ...
            _(types).forEach(function (item) {
                item.isAuthenticated = userService.isAuthenticated();
            ...
            dashboardData.getPageSize().then(function (data) {
                // pullDashboards();
                //routing default tab to product dashboard product
                setType("product");
            });
        })();
        ...
        function open(dashboardId , collectorItemId) {
            //pass Build Collector-Items Id also 
            //if Collector-Items Id is present:
            //1. Route to PipelineOverview modal
            //2. Route to Dashboard if Collector-Items Id not present 
            if(collectorItemId){
                /* $uibModal.open({
                    templateUrl: 'app/dashboard/views/pipelineOverview.html',
                    controller: 'PipelineOverviewController',
                    controllerAs: 'ctrl',
                    resolve: {
                        collectorItemId: function () {
                          return collectorItemId;
                        }
                      }
                }); */
                $location.path('/collectorItem/'+dashboardId +'/' + collectorItemId);

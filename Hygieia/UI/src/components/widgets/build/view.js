...
(function () {
    ...
    BuildWidgetViewController.$inject = ['$scope', 'buildData', 'DisplayState', '$q', '$uibModal', 'versionFilter'];
    function BuildWidgetViewController($scope, buildData, DisplayState, $q, $uibModal, versionFilter) {
        var ctrl = this;
		...
		console.log('latestBuilds: ' + data);
            ...
            worker.latestBuilds(data, function (buildsToDisplay) {
                //$scope.$apply(function () {
                var index, tempData=[];
                var totalSize = _.isEmpty(buildsToDisplay) ? 0 : buildsToDisplay.length;
                for (index = 0; index < totalSize; ++index) {
                   if(buildsToDisplay[index].number === versionFilter) {
                      tempData.push(buildsToDisplay[index]);
                   }
                }
                
                if (!_.isEmpty(tempData)){
                    ctrl.recentBuilds = tempData;
                }
                else {

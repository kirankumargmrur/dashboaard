...
(function () {
    ...
    angular
        ...
        .controller('VersionDataAnalysisConfigController', VersionDataAnalysisConfigController);

    VersionDataAnalysisConfigController.$inject = ['modalData', '$scope', 'collectorData', '$uibModalInstance'];
    function VersionDataAnalysisConfigController(modalData, $scope, collectorData, $uibModalInstance) {
        var ctrl = this,
        ...
        $scope.getCodeQualityCollectors = function (filter) {
            return collectorData.itemsByType('versions', { "search": filter, "size": 20 }).then(function (response) {
                return response;
            });
        };
        ...
        collectorData.itemsByType('versions').then(processTestsResponse);
        collectorData.itemsByType('libraryPolicy').then(processOSSscanResponse);

        function loadSavedCodeQualityJob() {
            var codeQualityCollectorItems = component.collectorItems.VersionData,

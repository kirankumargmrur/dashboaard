(function () {
    ...
    function productViewController($scope, $document, $uibModal, $location, $q, $stateParams, $timeout, buildData, codeAnalysisData, collectorData, dashboardData, pipelineData, testSuiteData, productBuildData, productCodeAnalysisData, productCommitData, productSecurityAnalysisData, productTestSuiteData, cicdGatesData, paginationWrapperService) {
            ...
            _(ctrl.configuredTeams).forEach(function (configuredTeam) {
                ...
                pipelineData
                    ...
                    .then(function (response) {
                        ...
                        var tempOrderMap = ["Build"];
                        ctrl.orderedStages[collectId] = tempOrderMap;
                    }).then(processLoad);
            });
        };
        ...
        function openDashboard(item) {
            ...
            //need to get the dashboard details from paginationWrapperService and iterate for build Id using 
            //dashboard Id
            dashboardData.search().then(function(data){
                ...
                if(localDashboard){
                    for (var x = 0; x < localDashboard.length; x++) {
                        if(localDashboard[x].id === dashboardDetails.id){
                            if(localDashboard[x].collectorItemId){
                                $location.path('/collectorItem/'+dashboardDetails.id +'/' + localDashboard[x].collectorItemId);
                            } else{
                                $location.path('/dashboard/' + dashboardDetails.id);
                            }
